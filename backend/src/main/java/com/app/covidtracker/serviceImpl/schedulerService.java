...
public class schedulerService {
    ...
    @SchedulerLock(name = ScheduledTasks.INDIA_RESOURCES, lockAtLeastFor = "PT5M", lockAtMostFor = "PT15M")
    public void updateIndiaResources() throws Exception {
        log.info("Starting scheduled job: Updating Indian resources stats");
        indianStatsService.updateIndianResources();
        log.info("Finishing scheduled job: Updating Indian resources stats");
    }

    // Update every 8 hours
    @Scheduled(cron = "0 0 0/1 * * *")
    @SchedulerLock(name = ScheduledTasks.INDIA_STATES, lockAtLeastFor = "PT5M", lockAtMostFor = "PT7M")
    public void updateIndianStatesStats() throws Exception {
        log.info("Starting scheduled job: Updating Indian states stats");
        indianStatsService.updateIndianStatesStats();
        log.info("Finishing scheduled job: Updating Indian states stats");
    }

    // Updated based on Indian States. Hence a delay of 10 minutes
    @Scheduled(cron = "0 10 0/1 * * *")
    @SchedulerLock(name = ScheduledTasks.INDIA_ZONES, lockAtMostFor = "PT4M", lockAtLeastFor = "PT10S")
    public void updateIndianZones() throws Exception {
        log.info("Starting scheduled job: Updating Indian Zones");
        indianStatsService.updateIndianZones();
        log.info("Finishing scheduled job: Updating Indian Zones");
    }
}
