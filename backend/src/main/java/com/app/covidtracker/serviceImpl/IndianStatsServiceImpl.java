...
public class IndianStatsServiceImpl implements IndianStatsService {
    ...
    public void updateGeoCode() throws Exception {
        ...
        for (Resources resource: resources) {
            if (isNull(resource.getLng()) || isNull(resource.getLat())) {
                GeocodingResult[] results =  GeocodingApi.geocode(context,
                        String.join(" ", resource.getOrganisation(), resource.getCity(), resource.getState()))
                        .await();
                if (results.length > 0) {
                    resource.setLat(results[0].geometry.location.lat);
                    resource.setLng(results[0].geometry.location.lng);
                    resourcesRepository.save(resource);
                } else {
                    results =  GeocodingApi.geocode(context,
                            String.join(" ", resource.getCity(), resource.getState())).await();
                    if (results.length > 0) {
                        resource.setLat(results[0].geometry.location.lat);
                        resource.setLng(results[0].geometry.location.lng);
                        resourcesRepository.save(resource);
                    } else {
                        results = GeocodingApi.geocode(context, resource.getState()).await();
                        if (results.length > 0) {
                            resource.setLat(results[0].geometry.location.lat);
                            resource.setLng(results[0].geometry.location.lng);
                            resourcesRepository.save(resource);
